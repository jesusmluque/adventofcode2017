...
case class Operation(op: String, value: Int)
case class Condition(register: String, op: String, value: Int)
case class Register(name: String, value: Int)
case class Execution(register: Register, operation: Operation, condition: Condition)

val inputParsed = "\n".r.split(input)
  .toList.map("\\s".r.split(_).toList).map { ex =>
    val indexed = ex.toVector
    Execution(Register(indexed(0), 0), Operation(ex(1), ex(2).toInt), Condition(ex(4), ex(5), ex(6).toInt))
}

def run(executions: List[Execution]) = {
  def findRegister(regs: List[Register], name: String): Register = {
    regs.find(_.name == name).getOrElse(Register(name, 0))
  }
  def updateRegs(regs: List[Register], reg: Register): List[Register] = {
    val left = regs.takeWhile(_.name != reg.name)
    val right = regs.takeRight(regs.length - left.length - 1)
    val newL = left ++ List(reg) ++ right
    newL
  }
  def doOper(regs: List[Register], op:Operation, reg: Register): List[Register] = {
    op match {
      case Operation(o, v) if o == "inc"
        => updateRegs(regs, Register(reg.name, reg.value + v))
      case Operation(o, v) if o == "dec"
      => updateRegs(regs, Register(reg.name, reg.value - v))
    }
  }
  executions.foldLeft(List[Register]()) { (registerList, op) =>
    op.condition match {
      case Condition(reg, "<", value)
        if findRegister(registerList, reg).value < value => doOper(registerList, op.operation, findRegister(registerList, op.register.name))
      case Condition(reg, "<=", value)
        if findRegister(registerList, reg).value <= value => doOper(registerList, op.operation, findRegister(registerList, op.register.name))
      case Condition(reg, ">", value)
        if findRegister(registerList, reg).value > value => doOper(registerList, op.operation, findRegister(registerList, op.register.name))
