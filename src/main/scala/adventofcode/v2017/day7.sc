...
val list = """gbyvdfh (155) -> xqmnq, iyoqt, dimle
oweiea (97)
szhxrs (14)
pjvwsiw (23)
...
case class Program(name: String, weight: String, children:List[String])
val lines = list.split("\n").toList.map { line =>
  "->".r.split(line)
        .toList
        .flatMap { l =>
          ",\\s|\\s".r.split(l).filter(_ != "")
        }.map { e =>
          if (e.startsWith("(")) e.filter(c => c != '(' && c != ')') else e
        }
}.map { line => line match {
  case name :: w :: children => Program(name, w, children)
  case name :: w :: List() => Program(name, w, List[String]())
  }
}
def findTop(p: List[Program]) = {
  val notATopList = (p |@| p) { (p1, p2) =>
    if (p2.children contains(p1.name))
      p1.name
    else
      ""
  }
  p.find { p =>
      !(notATopList contains(p.name))
    }
}
findTop(lines).get.name
